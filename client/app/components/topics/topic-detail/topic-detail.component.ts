...
export class TopicDetailComponent implements OnInit {
  ...
  deleteItem() {
    ...
    const dialogRef = this.dialog.open(ConfirmDeleteTopicDialog, {
      width: '250px',
      data: {id: this.topicId, title: this.protectedData.title},
    });
    // alter user role after closed, if confirmed
    dialogRef.afterClosed().subscribe((results) => {
      if (results == true) {
        this._api.deleteTypeRequest('topics/' + this.topicId).subscribe((res:any) => {
          if (res.message == 'Topic was deleted successfully!') {
          ...
          this._snackBar.open('Problem deleting item!', '', { duration: 4000 });
        });
      }
    });
    return true;
...
@Component({
  selector: 'confirm-delete-topic-dialog',
  templateUrl: 'confirm-delete-topic.dialogue.html',
})
export class ConfirmDeleteTopicDialog {

  constructor(
    public dialogRef: MatDialogRef<ConfirmDeleteTopicDialog>,
